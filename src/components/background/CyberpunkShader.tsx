...
/* ═══ GLSL fragment shader — cyberpunk grid ═══ */
...
// cyberpunk mesh gradient — deep black with cyan/violet hotspots
vec3 meshGradient(vec2 uv){
  vec2 p0=vec2(-0.70,-0.45), p1=vec2(0.75,-0.35), p2=vec2(-0.65,0.65), p3=vec2(0.80,0.55);
  vec3 c0=vec3(0.01,0.04,0.08);   // near-black blue
  vec3 c1=vec3(0.04,0.12,0.20);   // deep cyan tint
  vec3 c2=vec3(0.05,0.02,0.12);   // deep violet tint
  vec3 c3=vec3(0.02,0.08,0.16);   // dark teal
  float e=2.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 baseDeep = vec3(0.02,0.02,0.05);
  vec3 baseTint = vec3(0.03,0.04,0.08);
  ...
  // cyberpunk grid colors: cyan thin, violet major
  vec3 lineThin  = vec3(0.165,0.776,0.973);  // #2ac6f8 cyan
  vec3 lineMajor = vec3(0.545,0.361,0.965);  // #8B5CF6 violet

  // subtle pulse on grid brightness
  float pulse = 0.85 + 0.15 * sin(t * 0.8);

  vec3 col = bg
           + lineThin  * thin  * 0.22 * pulse
           + lineMajor * major * 0.42 * pulse;

  // ASCII glyphs on major intersections
  ...
  if (ASCII_AMT > 0.001) {
    ...
    vec3 asciiColor = mix(vec3(0.165,0.776,0.973), vec3(0.545,0.361,0.965), 0.3);
...
function ShaderCanvas({ fragSource }: { fragSource: string }) {
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => { if (e.button === 0) mouseRef.current.l = 1; };
    const onUp = () => { mouseRef.current.l = 0; };
    ...
    const getDpr = () => {
      ...
      return Math.max(1, Math.min(maxDpr, sys));
    };
    ...
    if (!vs) { console.error("Vert:", vsLog); return cleanup; }
    const { shader: fs, log: fsLog } = safeCompile(gl, gl.FRAGMENT_SHADER, fragSource);
    if (!fs) { console.error("Frag:", fsLog); gl.deleteShader(vs); return cleanup; }
    const linked = safeLink(gl, vs, fs);
    gl.deleteShader(vs);
    gl.deleteShader(fs);
    if (!linked.program) { console.error("Link:", linked.log); return cleanup; }
    ...
    function tick(now: number) {
      ...
      try {
        ...
        if (uMouse) {
          ...
          gl.uniform4f(uMouse, m.x * dpr, m.y * dpr, m.l, 0);
        }
...
/* ═══ Export ═══ */
export function CyberpunkShader() {
  return (
    <div
      aria-hidden="true"
      style={{
