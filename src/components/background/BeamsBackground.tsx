...
export function BeamsBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number) {
      ...
      beam.hue = 185 + (index * 85) / BEAM_COUNT;
  ...
  if (!mounted) {
    return <div className={cn("fixed inset-0 z-0 overflow-hidden bg-[#0a0a0f]", className)} />;
  }
  ...
  return (
    <div className={cn("fixed inset-0 z-0 overflow-hidden", className)}>
      ...
      />
      <motion.div
        className="absolute inset-0 bg-[#0a0a0f]/5"
